...
def integrate_cfg(cfg, command_line_args, input_dir, output_dir):
    ...
    cfg.DIR.IN_PATH = input_dir
    cfg.DIR.OUT_PATH = output_dir
...
def process_dir(cfg, file_info, video_name):
    indir = cfg.DIR.IN_PATH
    outdir = cfg.DIR.OUT_PATH
    ...
    file_paths = [os.path.join(indir, info_i.split()[0]) for info_i in file_info]

    # set simulator
    sim = EventSim(cfg=cfg, output_folder=outdir, video_name=video_name)
...
if __name__ == "__main__":
    ...
    for seq in args.seqs:
        input_dir = os.path.join(args.root_dir, seq, 'Src_Img', '1')
        output_dir = os.path.join(args.root_dir, seq, 'events')

        # 确保输出目录存在
        os.makedirs(output_dir, exist_ok=True)

        if is_valid_dir(input_dir):
            cfg_current = integrate_cfg(cfg, args, input_dir, output_dir)
            ...
            # simulation
            process_dir(cfg=cfg_current, file_info=video_info, video_name=seq)
        else:
            print(f"Skipping {input_dir}, as 'info.txt' was not found.")
